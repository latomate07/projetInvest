...
class Users extends Model
{
    ...
    */
    ...
    public function signin ($userfullname, $userpseudo, $usermail, $userRole, $userpassword, $userjoindate, $userCountry) 
    {
        if (!empty($userfullname) && !empty($userpseudo) && !empty($usermail) && !empty($userRole) && !empty($userpassword) && !empty($userjoindate)) {
            $query = $this->pdo->prepare("INSERT INTO {$this->table} (userFullName, userPseudo, userMail, userPassword, userRole, userJoinDate, userCountry) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $query->execute([$userfullname, $userpseudo, $usermail, $userpassword, $userRole, $userjoindate, $userCountry]);
             ...
             $notification->sendNotifications($sujet, $contenu, $receiver_id, $date, $seen);
